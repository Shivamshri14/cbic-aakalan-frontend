...
const ComparativeReport = ({
  ...
  const [datacustoms7, setDatacustoms7] = useState([]);
  const [datacustoms8, setDatacustoms8] = useState([]);
  const [datacustoms9, setDatacustoms9] = useState([]);
  const [datacustoms10, setDatacustoms10] = useState([]);
  const [datacustoms11, setDatacustoms11] = useState([]);
  const [datacustoms12, setDatacustoms12] = useState([]);
  ...
  const previousmonth6 = dayjs(selectedDate).subtract(6, "month").format("YYYY-MM-DD");
  const date7 = dayjs(previousmonth6).format("MMMM YYYY");
  const previousmonth7 = dayjs(selectedDate).subtract(7, "month").format("YYYY-MM-DD");
  const date8 = dayjs(previousmonth7).format("MMMM YYYY");
  const previousmonth8 = dayjs(selectedDate).subtract(8, "month").format("YYYY-MM-DD");
  const date9 = dayjs(previousmonth8).format("MMMM YYYY");
  const previousmonth9 = dayjs(selectedDate).subtract(9, "month").format("YYYY-MM-DD");
  const date10 = dayjs(previousmonth9).format("MMMM YYYY");
  const previousmonth10 = dayjs(selectedDate).subtract(10, "month").format("YYYY-MM-DD");
  const date11 = dayjs(previousmonth10).format("MMMM YYYY");
  const previousmonth11 = dayjs(selectedDate).subtract(11, "month").format("YYYY-MM-DD");
  const date12 = dayjs(previousmonth11).format("MMMM YYYY");

  const shouldDisableYear = (year) => year.year() < 2023;
  ...
  const handleChange = (e) => onSelectedOption(e.target.value);
  const handleChange1 = (e) => onSelectedOption1(e.target.value);
  ...
  const fetchData = async () => {
    try {
      ...
      const months = [newdate, previousmonth1, previousmonth2, previousmonth3, previousmonth4,
        previousmonth5, previousmonth6, previousmonth7, previousmonth8,
        previousmonth9, previousmonth10, previousmonth11];
      ...
      const getMonthData = async (month) => {
        ...
        const excludedZones = [
          "DG NORTH", "DG WEST", "DG EAST", "DG SOUTH", "DG (HQ)", "DRI DG"
        ];
        ...
        reducedAllData.sort((a, b) => a.zone_name.localeCompare(b.zone_name));
        ...
        return reducedAllData.map((item, index) => ({ ...item, s_no: index + 1 }));
      };
      ...
      setData7(allMonthData[6]);
      setData8(allMonthData[7]);
      setData9(allMonthData[8]);
      setData10(allMonthData[9]);
      setData11(allMonthData[10]);
      setData12(allMonthData[11]);
    } catch (error) {
  ...
  const fetchDatacustoms = async () => {
    try {
      ...
      setDatacustoms7(allMonthData[6]);
      setDatacustoms8(allMonthData[7]);
      setDatacustoms9(allMonthData[8]);
      setDatacustoms10(allMonthData[9]);
      setDatacustoms11(allMonthData[10]);
      setDatacustoms12(allMonthData[11]);
    } catch (error) {
      console.error("Error fetching data:", error);
    }
  };
  ...
  useEffect(() => {
    ...
    const fetchAllData = async () => {
      try {
        setLoading(true);
        await Promise.all([fetchData(), fetchDatacustoms()]);
  ...
  const columns = [
    ...
    { key: "sub_parameter_weighted_average_10", label: `${date10}` },
    { key: "sub_parameter_weighted_average_9", label: `${date9}` },
    { key: "sub_parameter_weighted_average_8", label: `${date8}` },
    { key: "sub_parameter_weighted_average_7", label: `${date7}` },
    { key: "sub_parameter_weighted_average_6", label: `${date6}` },
    { key: "sub_parameter_weighted_average_5", label: `${date5}` },
    { key: "sub_parameter_weighted_average_4", label: `${date4}` },
    { key: "sub_parameter_weighted_average_3", label: `${date3}` },
    { key: "sub_parameter_weighted_average_2", label: `${date2}` },
    { key: "sub_parameter_weighted_average_1", label: `${date1}` },
  ];
  ...
  const maxLength = Math.max(data1.length, data2.length, data3.length, data4.length,
    data5.length, data6.length, data7.length, data8.length,
    data9.length, data10.length, data11.length, data12.length);

  for (let i = 0; i < maxLength; i++) {
    userData.push({
      s_no: data1[i]?.s_no || null,
      zone_name: data1[i]?.zone_name || null,
      sub_parameter_weighted_average_1: data1[i]?.sub_parameter_weighted_average || null,
      sub_parameter_weighted_average_2: data2[i]?.sub_parameter_weighted_average || null,
      sub_parameter_weighted_average_3: data3[i]?.sub_parameter_weighted_average || null,
      sub_parameter_weighted_average_4: data4[i]?.sub_parameter_weighted_average || null,
      sub_parameter_weighted_average_5: data5[i]?.sub_parameter_weighted_average || null,
      sub_parameter_weighted_average_6: data6[i]?.sub_parameter_weighted_average || null,
      sub_parameter_weighted_average_7: data7[i]?.sub_parameter_weighted_average || null,
      sub_parameter_weighted_average_8: data8[i]?.sub_parameter_weighted_average || null,
      sub_parameter_weighted_average_9: data9[i]?.sub_parameter_weighted_average || null,
      sub_parameter_weighted_average_10: data10[i]?.sub_parameter_weighted_average || null,
      sub_parameter_weighted_average_11: data11[i]?.sub_parameter_weighted_average || null,
      sub_parameter_weighted_average_12: data12[i]?.sub_parameter_weighted_average || null,
    });
  }

  const userDatacustoms = [];
  const maxLengthcustoms = Math.max(datacustoms1.length, datacustoms2.length, datacustoms3.length,
    datacustoms4.length, datacustoms5.length, datacustoms6.length,
    datacustoms7.length, datacustoms8.length, datacustoms9.length,
    datacustoms10.length, datacustoms11.length, datacustoms12.length);

  for (let i = 0; i < maxLengthcustoms; i++) {
    userDatacustoms.push({
      s_no: datacustoms1[i]?.s_no || null,
      zone_name: datacustoms1[i]?.zone_name || null,
      sub_parameter_weighted_average_1: datacustoms1[i]?.sub_parameter_weighted_average || null,
      sub_parameter_weighted_average_2: datacustoms2[i]?.sub_parameter_weighted_average || null,
      sub_parameter_weighted_average_3: datacustoms3[i]?.sub_parameter_weighted_average || null,
      sub_parameter_weighted_average_4: datacustoms4[i]?.sub_parameter_weighted_average || null,
      sub_parameter_weighted_average_5: datacustoms5[i]?.sub_parameter_weighted_average || null,
      sub_parameter_weighted_average_6: datacustoms6[i]?.sub_parameter_weighted_average || null,
      sub_parameter_weighted_average_7: datacustoms7[i]?.sub_parameter_weighted_average || null,
      sub_parameter_weighted_average_8: datacustoms8[i]?.sub_parameter_weighted_average || null,
      sub_parameter_weighted_average_9: datacustoms9[i]?.sub_parameter_weighted_average || null,
      sub_parameter_weighted_average_10: datacustoms10[i]?.sub_parameter_weighted_average || null,
      sub_parameter_weighted_average_11: datacustoms11[i]?.sub_parameter_weighted_average || null,
      sub_parameter_weighted_average_12: datacustoms12[i]?.sub_parameter_weighted_average || null,
    });
  }
  ...
  const handleExport = () => {
    ...
    for (let i = 11; i >= 0; i--) {
      ...
      months.push(monthDate.format("MMMM YYYY"));
    }

    const dataToExport = exportData.map((user) => {
      const monthData = {};
      months.forEach((month, index) => {
        const dataField = `sub_parameter_weighted_average_${12 - index}`;
        monthData[month] = user[dataField] || "N/A";
      });
  ...
  const exportToXLS = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Monthly Scores");
    const fileName = selectedOption === "CGST"
      ? "cgst_comparative_report.xlsx"
      : "customs_comparative_report.xlsx";
